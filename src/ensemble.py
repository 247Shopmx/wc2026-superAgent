...
class EnsembleTrainer:
    ...
    def train_ml_models(
        self, X_train: pd.DataFrame, y_train: np.ndarray | pd.Series
    ) -> None:
        """Train ML classifiers with calibration.
        
        Args:
            X_train: Feature matrix.
            y_train: Target labels (numpy array or pandas Series).
        """
    ...
    def predict_ensemble(
        self, features: pd.DataFrame, lambda_home: float, lambda_away: float
    ) -> Dict[str, float]:
        """Generate weighted ensemble probabilities.
        
        Args:
            features: Feature matrix with shape (1, 7).
        ...
        (
            ...
            p_draw_dc,
            p_away_dc,
        ) = self.dixon_coles.get_match_outcome_probs(lambda_home, lambda_away)
